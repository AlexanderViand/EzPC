...
__global__ void aesWrapper(AESBlock *ss0, int N, AESGlobalContext gaes) {
  ...
  if (threadId < N) {
    ...
    applyAESPRGFourTimes(&saes, (u32 *)&ss2_l, (u32 *)ct, (u32 *)&ct[1],
                         (u32 *)&ct[2], (u32 *)&ct[3]);
    printf("Encrypt four=\n");
    printAESBlock(ct);
    printAESBlock(&ct[1]);
...
int main() {
  initGPUMemPool();
  // Use a fixed seed instead of sytorch's PRNG
  osuCrypto::block aesSeed = osuCrypto::toBlock(0x1234567890ABCDEF, 0xFEDCBA0987654321);
  ...
  int N = 1;
  auto d_aesSeed = (AESBlock *)moveToGPU((u8 *)&aesSeed, N * sizeof(AESBlock),
                                         (Stats *)NULL);
  AESGlobalContext g;
  initAESContext(&g);
